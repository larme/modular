...
# Hopper-oriented benchmark for MLA (Multi-head Latent Attention) decode kernels.
...
def _get_flashinfer_mla_kernel_name_candidates() -> list[str]:
    """Return likely FlashInfer MLA kernel name patterns for Hopper (SM90)."""
    return ["fmhaSm90", "fmhaSm", "fmha"]


def _flashinfer_mla_is_supported_on_hopper() -> bool:
    """FlashInfer MLA baseline is currently unsupported on Hopper in this setup."""
    return False
...
def bench_flashinfer_trtllm(
    ...
    # Workspace buffer for FlashInfer MLA decode path.
    ...
    # Benchmark with CUPTI warmup across likely Hopper/Blackwell kernel names.
    time_s: float | None = None
    last_runtime_error: RuntimeError | None = None
    kernel_candidates = _get_flashinfer_mla_kernel_name_candidates()
    for kernel_name in kernel_candidates:
        try:
            ...
            assert isinstance(measured, float)  # Single kernel_name returns float
            time_s = measured
            break
    ...
    if time_s is None:
        ...
        print(
            "Warning: kineto profiling failed to match MLA kernel names. "
            f"Tried: {candidates}. Falling back to no-kineto timing."
        )
        if last_runtime_error:
            print(f"Last kineto error: {last_runtime_error}")
        run_kernel()
        torch.cuda.synchronize()
        return 1.0, total_bytes
...
def bench_mla_decode(
    ...
    Args:
        ...
        q_len_per_request: Query length per request (1 for decode, >1 for chunked prefill)
        backend: Backend for FlashInfer.
    ...
    if engine == "flashinfer":
        if not _flashinfer_mla_is_supported_on_hopper():
            raise RuntimeError(
                "FlashInfer MLA benchmark is currently not supported on Hopper."
            )

...
# Hopper decode benchmark uses FlashInfer TRTLLM path as the external baseline.
...
def bench_flashinfer(
    ...
    Args:
        ...
        Uses the Hopper decode backend configuration (TensorRT-LLM path).
    """
    ...
    # Hopper TensorRT-LLM backend expects [num_blocks, 2, num_kv_heads, block_size, head_dim].
    ...
    time_s = bench_kineto(
        run_kernel,
        # TensorRT-LLM backend uses this kernel name even in decode.
        kernel_names="BatchPrefillWithPagedKVCacheKernel",
        num_tests=100,
